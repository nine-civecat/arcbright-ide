...
import './media/chatArcBright.css';
...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	render(parent: HTMLElement): void {
		...
		// ArcBright dark theme CSS custom properties
		this.container.style.setProperty('--arcbright-void', '#08090a');
		this.container.style.setProperty('--arcbright-s1', '#0f1011');
		this.container.style.setProperty('--arcbright-s2', '#191a1b');
		this.container.style.setProperty('--arcbright-s3', '#222326');
		this.container.style.setProperty('--arcbright-jade', '#18a058');
		this.container.style.setProperty('--arcbright-jg', 'rgba(24,160,88,0.12)');
		this.container.style.setProperty('--arcbright-border', 'rgba(255,255,255,0.06)');
		this.container.style.setProperty('--arcbright-tx', '#f7f8f8');
		this.container.style.setProperty('--arcbright-tx2', '#a0a4ac');
		this.container.style.setProperty('--arcbright-tx3', '#6b6f78');
		this.container.style.setProperty('--arcbright-cin', '#d03050');
		this.container.style.setProperty('--arcbright-amb', '#f0a020');
		this.container.style.setProperty('--arcbright-cy', '#2080f0');
		this.container.classList.add('arcbright-widget');
		...
		try {
			...
			if (!numItems) {
				...
				if (this.chatEntitlementService.anonymous && !this.chatEntitlementService.sentiment.completed) {
					const providers = product.defaultChatAgent?.provider || { default: { name: 'Arcbright' } };
					additionalMessage = new MarkdownString(localize({ key: 'settings', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with {0} Arcbright, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3}).", providers.default.name, providers.default.name, product.defaultChatAgent?.termsStatementUrl || 'https://arcbright.dev/terms', product.defaultChatAgent?.privacyStatementUrl || 'https://arcbright.dev/privacy'), { isTrusted: true });
				} else {
