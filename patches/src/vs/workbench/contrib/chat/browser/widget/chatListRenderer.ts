...
const COPILOT_USERNAME = 'Arcbright';
...
export class ChatListItemRenderer extends Disposable implements ITreeRenderer<ChatTreeItem, FuzzyScore, IChatListItemTemplate> {
	...
	private renderChatTreeItem(element: ChatTreeItem, index: number, templateData: IChatListItemTemplate): void {
		...
		// ArcBright bubble direction
		templateData.rowContainer.classList.toggle('arcbright-bubble-right', isRequestVM(element));
		templateData.rowContainer.classList.toggle('arcbright-bubble-left', isResponseVM(element));
		// ArcBright animation classes for thinking/loading state
		if (isResponseVM(element) && !element.isComplete && !element.isCanceled) {
			templateData.rowContainer.classList.add('arcbright-thinking');
			templateData.value.classList.add('hl-glow', 'hl-border');
		} else {
			templateData.rowContainer.classList.remove('arcbright-thinking');
			templateData.value.classList.remove('hl-glow', 'hl-border');
		}
		...
		// ArcBright: color assistant username jade
		if (isResponseVM(element)) {
			templateData.username.classList.add('arcbright-agent-name');
		} else {
			templateData.username.classList.remove('arcbright-agent-name');
		}
	...
	private renderDetail(element: IChatResponseViewModel, templateData: IChatListItemTemplate): void {
		...
		if (element.agentOrSlashCommandDetected) {
			...
			// ArcBright: add hl-cursor for thinking state
			templateData.detail.classList.add('hl-cursor');
		} else {
			templateData.detail.classList.remove('hl-cursor');
		}
	}
	...
	private renderAvatar(element: ChatTreeItem, templateData: IChatListItemTemplate): void {
		...
		// ArcBright diamond avatar shape
		templateData.avatarContainer.classList.toggle('arcbright-avatar', true);
	}
