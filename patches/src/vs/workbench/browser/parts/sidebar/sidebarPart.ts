...
export class SidebarPart extends AbstractPaneCompositePart {
	...
	protected getCompositeBarOptions(): IPaneCompositeBarOptions {
		return {
			...
			compact: true,
			forcePinnedIds: ActivitybarPart.HERMES_PINNED_IDS,
		};
	}
	...
	/**
	 * Show detail panel overlay in the sidebar — minimal ArcBright feature.
	 * Dispatches to the active pane composite if it supports detail.
	 */
	showDetail(id?: string): void {
		const active = this.getActivePaneComposite();
		if (active && typeof (active as any).showDetail === 'function') {
			(active as any).showDetail(id);
		}
	}
}
