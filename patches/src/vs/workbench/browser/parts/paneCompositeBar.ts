...
export interface IPaneCompositeBarOptions {
	...
	/** IDs that are always forced to be pinned (e.g. Hermes containers) */
	readonly forcePinnedIds?: string[];
}
...
export class PaneCompositeBar extends Disposable {
	...
	private updateCompositeBarItemsFromStorage(retainExisting: boolean): void {
		...
		// Force-pin required IDs (e.g. Hermes containers)
		const forceIds = this.options.forcePinnedIds;
		if (forceIds && forceIds.length > 0) {
			for (const item of newCompositeItems) {
				if (forceIds.includes(item.id)) {
					item.pinned = true;
				}
			}
		}
